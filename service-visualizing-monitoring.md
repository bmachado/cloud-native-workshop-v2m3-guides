...
In the namespace selector at the top, enter your **{{OPENSHIFT_USER_NAME}}-bookinfo** application and press enter to filter to only show your bookinfo project.
...
Let's see detailed metrics of the **productpage** service. Click on **productpage.{{OPENSHIFT_USER_NAME}}-bookinfo.svc.cluster.local** and the service dashboard will look similar to this:
...
> You should select your own {{OPENSHIFT_USER_NAME}}-bookinfo in the `Namespace` selector at the top to avoid noise from other workloads on the cluster!
...
metadata:
  ...
  generation: 1
  name: reviews
  namespace: {{OPENSHIFT_USER_NAME}}-bookinfo
